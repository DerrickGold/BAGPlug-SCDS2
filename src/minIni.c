...
static int getkeystring(FILE_STRUCT *fp, const TCHAR *Section, const TCHAR *Key,
                        int idxSection, int idxKey, TCHAR *Buffer, int BufferSize)
{
  ...
  if (len > 0 || idxSection >= 0) {
    idx = -1;
    do {
      if (!fgets(LocalBuffer, INI_BUFFERSIZE, fp))
        return 0;
  ...
  idx = -1;
  do {
    if (!fgets(LocalBuffer,INI_BUFFERSIZE,fp) || *(sp = skipleading(LocalBuffer)) == '[')
      return 0;
    sp = skipleading(LocalBuffer);
 ...
 */
inline int ini_gets(const TCHAR *Section, const TCHAR *Key, const TCHAR *DefValue,
 ...
 */
inline long ini_getl(const TCHAR *Section, const TCHAR *Key, long DefValue, const TCHAR *Filename)
{
  TCHAR buff[64];
 ...
 * \return            the number of characters copied into the supplied buffer
 */
inline int  ini_getsection(int idx, TCHAR *Buffer, int BufferSize, const TCHAR *Filename)
{
  //FILE_STRUCT fp;
  int ok = 0;
 ...
 * \return            the number of characters copied into the supplied buffer
 */
inline int  ini_getkey(const TCHAR *Section, int idx, TCHAR *Buffer, int BufferSize, const TCHAR *Filename)
{
  //FILE_STRUCT fp;
  int ok = 0;
...
static void writesection(TCHAR *LocalBuffer, const TCHAR *Section, FILE_STRUCT *fp)
{
  TCHAR *p;
  ...
  if (Section != NULL && _tcslen(Section) > 0) {
    ...
    *p++ = ']';
	...
	fputs(LocalBuffer,fp);
  } /* if */
}

static void writekey(TCHAR *LocalBuffer, const TCHAR *Key, const TCHAR *Value, FILE_STRUCT *fp)
{
  TCHAR *p;
...
static void write_quoted(const TCHAR *Value, FILE_STRUCT *fp)
{
  TCHAR s[3];
  int idx;
...
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, const TCHAR *Filename)
{
 ...
 // FILE_STRUCT wfp;
  ...
  if (*sp == '[') {
    ...
    if (Key!=NULL && Value!=NULL) {
     ...
     // ini_write(INI_LINETERM INI_LINETERM, &wfp); /* put a blank line between the current and the next section */
	  fputs(INI_LINETERM INI_LINETERM,wfp);
    } /* if */
  ...
  while (fgets(LocalBuffer, INI_BUFFERSIZE, rfp)) {
    sp = skipleading(LocalBuffer);
    if (_tcslen(sp) > 0) {
      if (*sp == '[')
        fputs(INI_LINETERM, wfp);
